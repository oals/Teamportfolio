...
<div class="main">
        <div class="container mt-5">
          <div class="w-75 m-auto">
            <h4 class="nameLabel" style="font-weight: 600;">공지사항</h4>
          </div>
            ...
            <table class="table">
              <thead class="table-light">
                <tr>
                  ...
                  <th scope="col" style="width: 60%" id="center">제목</th>
                  <th scope="col" style="width: 15%" id="center">글쓴이</th>
                  <th scope="col" style="width: 15%" id="center">작성일</th>
                </tr>
              </thead>
              <tbody>
                <c:forEach var="item" items="${a_boardlist}" step="1">
              
              
                <tr>
                  <th scope="row">${item.article_No}</th>
                  <td>
                    <a href="${ctxPath}/noticeSelect.do?article_No=${item.article_No}&admin_id=${item.admin_id}&title=${item.title}&content=${item.content}&write_date=${item.write_date}">${item.title}</a>
                  </td>
                  <td id="center">${item.admin_id}</td>
 		...
 		<nav aria-label="...">
                    <ul class="pagination">
                       	<c:forEach var="page" begin="1" end="${lastPage}" step="1" >
		                     <!--  section 2이상이고 1page이면 이전 버튼 활성화 -->	
	                       	 <c:if test="${section > 1 && page==1 }">
		                      	<li class="page-item ">
		                        	<a class="page-link" href="${ctxPath}/noticeList.do?pageBlock=${section-1}&pageNum=1">이전</a>
		                     	</li>
		                     </c:if>  
		                     ...
		                     <c:choose>
	   	                    	<c:when test="${page==pageNum}">
	   	                    	 <li class="page-item  disabled" aria-current="page">
			                      	<a class="page-link" href="${ctxPath}/noticeList.do?pageBlock=${section}&pageNum=${page}">${(section-1)*10 + page}</a>
			                     </li>
	   	                    	</c:when>
	   	                    	<c:otherwise>
	   	                    	 <li class="page-item ">
                        ...
                        <!--  다음 버튼 : 현재 section번호가 전체 section 번호 보다 작은 경우만 표시-->
	                      ...
	                      <li class="page-item">
	                        <a class="page-link" href="${ctxPath}/noticeList.do?pageBlock=${section+1}&pageNum=1">다음</a>
	                      </li>
                      	</c:if>
                    </ul>
                </nav>
        ...
        <script type="text/javascript">
	        ...
	        function noticeSelect(){
	    		location.href="${ctxPath}/noticeSelect.do"
	    	}
        	function noticeRegister(){
        		location.href="${ctxPath}/noticeRegister.do"
        	}
